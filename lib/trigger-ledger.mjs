...
if (process.argv[1] && process.argv[1].endsWith("trigger-ledger.mjs")) {
  ...
  if (soft.length) {
    ...
    console.log(`ℹ️ the set of competing descriptions changed after the last measurement — ` +
  ...
  if (!stale.length) {
    console.log(`✓ trigger measurements are current for ${withEval.length} skill(s) of ${total}` +
      (noEval ? ` — the other ${noEval} have NO .eval.mjs of their own, they are outside this ` +
