...
function fixture(pkgJson, casesFile) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "rpp-cases-")));
  writeFileSync(join(root, "package.json"), JSON.stringify(pkgJson));
  if (casesFile) {
    mkdirSync(join(root, ".claude", "lib"), { recursive: true });
    writeFileSync(
      join(root, ".claude", "lib", "skill-trigger-cases.local.mjs"),
      casesFile,
    );
  }
  return root;
...
soft("I. every case is shaped and every collider resolves", () => {
  for (const c of CASES) {
    ...
    assert.equal(
      c.prompts.length,
      4,
      `${c.skill}: ${c.prompts.length} prompts, expected 4`,
    );
    for (const sib of c.collides)
      assert.ok(
        BY_SKILL.has(sib),
        `${c.skill} collides with unknown skill "${sib}"`,
      );
  }
});
...
soft("I. irrelevantFor still yields a full set for a one-collider case", () => {
  ...
  assert.equal(
    irrelevantFor(one.skill).length,
    4,
    `${one.skill}: irrelevant set is short`,
  );
});
...
soft(
  ...
  () => {
    ...
    assert.equal(
      Number(out.slice(3)),
      CASES.length,
      "the package table changed size with no input",
    );
  },
);
...
soft("II. declared and missing THROWS, naming the declaration", () => {
  const out = loadAt(
    fixture({ "research-paper-pipeline": { triggerCases: "nope.mjs" } }, null),
  );
  assert.match(out, /^THROW /, `a typoed declaration loaded quietly: ${out}`);
  assert.match(
    out,
    /nope\.mjs/,
    `the throw does not name the declared path: ${out}`,
  );
});
...
soft("II. a consumer may not redefine a case this package owns", () => {
  ...
  assert.match(
    out,
    new RegExp(CASES[0].skill),
    `the throw does not name the skill: ${out}`,
  );
});
