...
process.exit(
  runMutations({
    ...
    cases: [
      {
        // Возврат к тому, как было до 17.09: всё, что не убито, объявляется выжившим — и
        // «патч не лёг» становится неотличим от «тест слаб». Мутация ловится ТОЛЬКО третьим
        // исходом харнесса; первые два проходят при ней без изменений.
        name: "«не применилась» снова сваливается в «выжила»",
        harness: HARNESS,
        expect: "вердикт обязан называться непригодностью, а не выживанием",
        disables:
          "различение двух вердиктов, которые отправляют читателя в противоположные стороны: " +
          "survived — усиливать тест, unusable — чинить мутацию. Смешав их, движок посылает " +
          "переписывать исправный харнесс",
