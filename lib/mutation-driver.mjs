 ...
 * own published API.
 *
 * The rule "mechanism → vigiles" was applied too bluntly, and this is the correction worth keeping:
 ...
 * data about OUR checkers and belongs next to them (root CLAUDE.md: "mechanism — in vigiles,
 * data — in mine"; the same line separates mechanism from data inside this repo). This file knows
...
const runners = {
  node: (harness, root) =>
    spawnSync("node", [harness], { cwd: root, encoding: "utf8" }),
  vigiles: (harness, root) =>
    spawnSync("npx", ["vigiles", "test", harness], {
      cwd: root,
      encoding: "utf8",
    }),
};
...
function probeHarness(runner, harness, root) {
  ...
  if (runner === "node") {
    ...
    return r.status === 0
      ? "green"
      : r.status === VIGILES_SKIP_EXIT
        ? "skip"
        : "red";
  }
  const strict = spawnSync("npx", ["vigiles", "test", harness, "--no-skip"], {
    cwd: root,
    encoding: "utf8",
  });
...
export function runMutations({ root, cases, runner = "node", cleanHarnesses }) {
  ...
  if (!run)
    throw new Error(
      `unknown runner ${JSON.stringify(runner)}; expected node or vigiles`,
    );
  if (!cases.length)
    throw new Error(
      "no mutation cases were passed — a runner with nothing to run reports success",
    );
  ...
  if (missing.length) {
    console.error(
      `refusing to run: harness file(s) do not exist:\n  ${missing.join("\n  ")}`,
    );
    return 2;
  }
  ...
  const probes = new Map(
    harnesses.map((h) => [h, probeHarness(runner, h, root)]),
  );
  ...
  if (redBefore.size) {
    console.log(
      `ℹ️  already red before any mutation: ${[...redBefore].map(base).join(", ")} — excluded from the restore check, and any case naming one is reported as UNJUDGEABLE.`,
    );
  }
  ...
  try {
    for (const c of cases) {
      ...
      for (const [file, find, replace] of c.edits) {
        ...
        if (next === src) {
          problem = "NO-OP (replacement equals original)";
          break;
        }
        writeFileSync(file, next);
      }
      if (problem) {
        rows.push([c.name, problem, c.disables]);
        restore();
        continue;
      }
      ...
      if (verdict !== "RED (expected case)") {
        ...
        if (second === "RED (expected case)")
          verdict = "RED (expected case, on retry)";
        else verdict = second;
      }
  ...
  for (const [name, verdict, disables] of rows)
    console.log(`${name.padEnd(width)}  ${verdict.padEnd(38)} ${disables}`);
  ...
  const cleanSet = (cleanHarnesses ?? harnesses).filter(
    (h) => !redBefore.has(h),
  );
  ...
  // 🔴 A MUTATION THAT NEVER LANDED IS NOT A SURVIVOR, and until 2026-09-17 this line said it
  // was: every row that did not start with `RED (expected` went into `survived`, including
  // `NOT FOUND`, `AMBIGUOUS` and `NO-OP` — cases where the file was never edited at all. The two
  // verdicts send a reader to opposite places: a survivor means STRENGTHEN THE TEST, an unusable
  // case means FIX THE MUTATION. Observed cost: a stale `find` string (the rule had been
  // rewritten) was reported as "1 mutation survived", and the next step would have been rewriting
  // a harness that was fine. The docstring two hundred lines above warns about exactly this
  // confusion — the summary contradicted it.
  const unusable = rows.filter(
    (r) =>
      r[1].startsWith("NOT FOUND") ||
      r[1].startsWith("AMBIGUOUS") ||
      r[1].startsWith("NO-OP"),
  );
  const survived = rows.filter(
    (r) => !r[1].startsWith("RED (expected") && !unusable.includes(r),
  );
  if (unusable.length)
    console.log(
      `🔴 ${unusable.length} mutation(s) UNUSABLE — the patch never landed, so nothing was ` +
        `measured about the test: ${unusable.map((r) => `${r[0]} [${r[1]}]`).join(", ")}`,
    );
  console.log(
    survived.length
      ...
      : // "each at its own assertion" is not decoration — a kill only counts when the harness went
        ...
        unusable.length
        ? `(${rows.length - unusable.length} of ${rows.length} mutations killed; the rest were never applied)`
        : `✓ all ${rows.length} mutations killed by the harness, each at its own assertion`,
  );
  return survived.length || unusable.length || stillRed.length ? 1 : 0;
}
