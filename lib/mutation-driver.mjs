...
function probeHarness(runner, harness, root) {
  // The node runner needs no probe: a skipping harness exits 77 itself, and that code is the
  // signal. It is only the `vigiles` wrapper that swallows it into a 0.
  if (runner === "node") {
    const r = runners.node(harness, root);
    return r.status === 0 ? "green" : r.status === 77 ? "skip" : "red";
  }
