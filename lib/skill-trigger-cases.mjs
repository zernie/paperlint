...
// consumer's three non-paper cases share the very wording of the request («напиши сообщение…»),
...
//     "research-paper-pipeline": { "triggerCases": ".claude/lib/skill-trigger-cases.local.mjs" }
//
...
import { CONFIG_KEY, consumerRoot } from "../skills/paper-pipeline/scripts/consumer.mjs";
...
async function consumerCases() {
  ...
  let declared;
  try {
    declared = JSON.parse(readFileSync(join(root, "package.json"), "utf8"))?.[CONFIG_KEY]
      ?.triggerCases;
  } catch {
    declared = undefined;
  }
...
${String(e?.message ?? e)}`,
  ...
  if (!Array.isArray(extra))
    throw new TypeError(`${CONFIG_KEY}: ${abs} must export an array named CASES`);
