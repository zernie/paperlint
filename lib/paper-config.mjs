...
/**
 * Keys removed in 3.0.0, each with what replaces it. A config that still has one is REFUSED with
 * this message rather than the generic "unknown key": the user did nothing wrong, the key was
 * valid yesterday, and the only useful answer is what to write instead.
 */
export const REMOVED_SETTINGS = Object.freeze({
  typographyDebt:
    "the per-paper typography debt is gone: paper/typography was split into paper/section-word, " +
    "paper/leading-zero, paper/figure-ref-style and bib/reachable-entry, and `paperlint lint --fix` " +
    "fixes the first three. Keep a deliberate exception with `% eslint-disable-next-line <rule> -- <why>`",
  authorListCommand:
    "the author-list check is paperlint's own command now: `npx paperlint authors <paper>` runs it " +
    "and records `authorsVerified: <date>` in the paper's PIPELINE-STATUS.md",
  docFields:
    'required review fields are a JSON Schema now: point "reviewSchema" at a schema file; it is ' +
    "applied together with paperlint's own (allOf). See docs/rules.md, review/frontmatter",
  reviewSince:
    "review findings are records in the review's frontmatter now (`findings:`); a review with no " +
    "`findings` key is not checked, so no date is needed. See docs/rules.md, review/frontmatter",
  minFindings:
    "review findings are records in the review's frontmatter now (`findings:`), and every open one " +
    "names its cause; there is no threshold. See docs/rules.md, review/frontmatter",
  causeMarker:
    "a finding's cause is a field now (`cause:` in the review's frontmatter), not a word in the " +
    "prose. See docs/rules.md, review/frontmatter",
});
...
export const SETTINGS_KEYS = Object.freeze({
  ...
  reviewSchema:
    "a JSON Schema file a review's frontmatter must ALSO satisfy — review/frontmatter",
  rules:
