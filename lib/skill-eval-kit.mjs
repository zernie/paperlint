 ...
 */
// ⚠️ TWO ENTRY POINTS, SPLIT BY COST (vigiles v16, #149): the root barrel `vigiles` is the
// free deterministic testing surface, `vigiles/eval` is everything that spends money, behind
// a `paid_` prefix. `vigiles/testing` no longer exists — importing it throws
// ERR_PACKAGE_PATH_NOT_EXPORTED. An import of it sat in this file on the `{ min }` branch until
// #67, where `n/no-missing-import` found it: a subpath nobody loads is read by a person, not
// executed, which is how it outlived the split.
import {
  assertTriggerRate,
