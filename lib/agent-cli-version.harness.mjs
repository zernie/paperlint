...
// ── 1. BINARY ABSENT: no throw, and `present` says so — the skip path of the caller.
{
  ...
  assert.equal(
    o.present,
    false,
    "a non-zero probe means the binary is not usable",
  );
  assert.equal(o.version, "", "and there is no version to report");
  assert.match(
    o.note,
    /not installed/,
    "the note must say WHY there is nothing to observe",
  );
...
//      2.1.273 while the assertions were written against 2.1.227.
{
  ...
  assert.equal(
    o.version,
    "2.1.273",
    "the version must be PARSED out of the line, not left raw",
  );
  assert.equal(
    o.raw,
    "2.1.273 (Claude Code)",
    "and the full line kept for the report",
  );
  assert.equal(
    o.characterized,
    CHARACTERIZED_CLI,
    "the characterized number rides along as a value",
  );
  assert.equal(
    o.drifted,
    true,
    "2.1.273 ≠ 2.1.227 is drift and must be computed, not eyeballed",
  );
  assert.ok(
  ...
  cases.push(
    "drifted CLI → version parsed, drifted:true, note names BOTH numbers",
  );
}
...
//      would be a constant rather than a comparison.
{
  const spawnSync = spawner({
    status: 0,
    stdout: `${CHARACTERIZED_CLI} (Claude Code)\n`,
    stderr: "",
  });
  const o = observeAgentCli({ spawnSync });
...
//      must THROW rather than return an empty observation.
{
  ...
  cases.push(
    'exit 0 + empty stdout (the `stdio: "ignore"` shape) → throws, does not report a blank',
  );
}
...
//      other path happened to fill stdout. So the options handed to the spawner are checked.
{
  ...
  assert.deepEqual(
    args,
    ["--version"],
    "the observation must ask for the version",
  );
  assert.equal(
    options.encoding,
    "utf8",
    "without an encoding the output is a Buffer nobody reads",
  );
  assert.notEqual(
    options.stdio,
    "ignore",
    '`stdio: "ignore"` is the discarded-output defect itself',
  );
...
//      the output format of the tool it exists to read.
{
  ...
  if (o.present) {
    assert.match(
      o.version,
      /^\d+\.\d+\.\d+$/,
      `the real CLI's version must parse; raw was ${JSON.stringify(o.raw)}`,
    );
    cases.push(
      `real \`claude\` binary present → observed ${o.version} (characterized ${o.characterized}${o.drifted ? ", DRIFTED" : ""})`,
    );
  } else {
    cases.push(
      "real `claude` binary absent on this machine → absent path exercised for real",
    );
  }
}
