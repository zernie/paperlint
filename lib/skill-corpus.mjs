...
// ⚠️ 2026-08-28: `rmSync` и `writeFileSync` отсюда убраны — их не звал никто. `rmSync` при этом
// НЕ безобиден как находка: `tmp` ниже — это `mkdtempSync`, который никогда не удаляется, то есть
// каждый импорт этой библиотеки оставляет каталог в `/tmp` (за один свод харнессов — два десятка).
// Уборку не дописываю в этой правке СОЗНАТЕЛЬНО: она меняет жизненный цикл каталога, в котором
// лежит `PIPELINE_LEDGER` всех дочерних прогонов, а правка сегодня — про включение линта, не про
// поведение харнессов. Записано в отчёт как открытый пункт, а не заметено импортом-призраком.
...
// Разбор разметки — парсером (`CLAUDE.md`, 2026-08-11). Здесь ПАДАЕМ, а не деградируем:
// этот модуль отвечает на вопрос «что в корпусе скиллов есть», и пустой список заголовков
// без парсера читался бы как «ни у одного скилла нет блока `Record the verdict`» — то есть
// как находка про корпус. Уверенный ноль в этой репе уже четырежды принимали за факт.
requireMarkdown();
...
export const tmp = mkdtempSync(join(tmpdir(), "skills-harness-"));
...
export function parseFm(fmText, where = "frontmatter") {
  try {
    ...
    throw new Error(
      ...
      // `cause` — не украшение: сообщение выше берёт `.split("\n")[0]`, то есть выбрасывает
      // строку и колонку, которые js-yaml печатает вторым абзацем. Без причины отладка сломанного
      // фронтматтера начинается с поиска места руками (2026-08-28, `preserve-caught-error`).
      { cause: e },
    );
  }
}
 ...
 *
 * 2026-08-11 — переведено на парсер (`CLAUDE.md`: «markdown разбираем парсером»). Тот же
 * off-by-one теперь НЕВЫРАЗИМ: у заголовка из парсера есть готовое смещение начала строки,
 * и границу блока не надо вычислять из позиции перевода строки. Заодно ушли два дефекта,
 * которые прежнее выражение имело молча: `#{1,3}` считал заголовком решётку внутри
 * ```-блока (скилл, ЦИТИРУЮЩИЙ чужой `## Run me` в примере, обрывал свой блок на цитате),
 * и `\n#{1,3} ` не видел заголовок с отступом в 1–3 пробела, который CommonMark считает
 * заголовком. Граница блока сохранена побайтово: конец = начало следующего заголовка.
 */
...
//
// 🔴 2026-08-28: НИЖЕ БЫЛО ЕЩЁ ДВЕ СТРОКИ, И ОНИ — НЕПРОВЕДЁННАЯ ЛОГИКА, А НЕ МУСОР.
//
//     const skills = (ONLY ? [ONLY] : SKILLS).map(load);
//     const skillDirs = new Set(readdirSync(SKILLS_DIR).filter((d) => existsSync(…)));
//
// Обе вычислялись и не читались НИКЕМ — ни здесь, ни на экспорте. Это остаток разделения
// 2026-08-11, когда межскилловые проверки уехали в `pipeline-corpus.harness.mjs`: тела
// проверок унесли, а их входные данные забыли. Абзац выше про «cross-skill assertions
// below» описывал именно тот, уехавший блок; после переезда НИЖЕ НЕТ НИ ОДНОЙ ПРОВЕРКИ,
// то есть комментарий обещал механизм, которого нет, — ровно то, за что в этой базе
// заведено правило «prose isn't policy».
//
// Вторая, менее очевидная цена: `.map(load)` — это не пустое вычисление. `load()` держит
// `assert.ok`, поэтому строка была НЕОБЪЯВЛЕННОЙ проверкой всего корпуса, выполнявшейся
// как побочный эффект `import` и стоившей чтения всех 22 `SKILL.md` при каждом импорте
// библиотеки. Именно от такой связи «через побочный эффект импорта» файл и уходил
// (см. шапку `skill-checks.mjs`). Настоящий, объявленный носитель этой проверки —
// `checkSkill()`, который её и делает, по одному скиллу и с внятным сообщением.
//
// `ONLY` остаётся: его читает проверка строкой выше — единственная, что тут уцелела.
