...
const { KINDS, RETIRED, ABSTENTIONS, record, parseGate } =
  await import("../skills/paper-pipeline/scripts/ledger.mjs");
export { KINDS, RETIRED, ABSTENTIONS, record, parseGate };
const { EXPECTED_GATES } =
  await import("../skills/paper-pipeline/scripts/status.mjs");
export { EXPECTED_GATES };
/** The skills EXPECTED_GATES names, with the `skill/check` rows collapsed onto their skill. */
export const GATE_SKILLS = new Set(
  EXPECTED_GATES.map((g) => parseGate(g).skill),
);
...
export const EXCLUDED = new Map([
  [
    "paper-status",
    "reports on the pipeline; produces no verdict ABOUT the paper, so it has no gate row",
  ],
]);
...
const wiredOnDisk = installedSkills(SKILLS_DIR).filter((d) =>
  PIPELINE_MARKER.test(readFileSync(join(SKILLS_DIR, d, "SKILL.md"), "utf-8")),
);
...
export function recordBlock(src) {
  ...
  const i = hs.findIndex(
    (h) => h.depth <= 3 && /record the verdict/i.test(h.text),
  );
  if (i < 0) return null;
...
export const allBold = (blk) => [
  ...new Set(
    [...blk.matchAll(/\*\*`?([A-Z][A-Z-]{2,})`?\*\*/g)].map((m) => m[1]),
  ),
];
export const namedKinds = (blk) =>
  allBold(blk).filter((v) => KINDS.includes(v));
export const namedRetired = (blk) => allBold(blk).filter((v) => RETIRED.has(v));
export const definedKinds = (blk) => [
  ...new Set(
    [...blk.matchAll(/\*\*`?([A-Z][A-Z-]{2,})`?\*\*\s*(?=[—(:]| \()/g)].map(
      (m) => m[1],
    ),
  ),
];
...
export const namedReasons = (blk) =>
  [
    ...new Set([...blk.matchAll(/`([a-z][a-z-]{3,})`/g)].map((m) => m[1])),
  ].filter((r) => ABSTENTIONS.has(r));
...
export const ADMISSION =
  /(almost always record|never returned a (negative|finding)|a generator is not a gate|only honest negative)/i;
