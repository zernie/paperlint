...
// `toolList` are called INSIDE `load()` (`skill-corpus.mjs:286,289`), and the direct import was a
...
export async function checkProseSkill(name, opts = {}) {
  ...
  soft(() => assert.ok(c.declared.length > 0, `${name}: empty allowed-tools — inherits everything`));
  ...
  if (deferred.length) throw new AggregateError(deferred, `${name}: ${deferred.length} findings`);
  recordCheck();
  return 1;
}
