...
// ── 3. ПАТЧ НЕ ЛЁГ ⇒ это НЕ выжившая мутация. Третий исход, и до 17.09 движок сваливал его
// во второй: всё, что не `RED (expected`, считалось survived — включая NOT FOUND, AMBIGUOUS и
// NO-OP, где файл не редактировался вовсе.
//
// 🔴 Два вердикта отправляют читателя в ПРОТИВОПОЛОЖНЫЕ стороны: survived значит УСИЛИТЬ ТЕСТ,
// unusable значит ПОЧИНИТЬ МУТАЦИЮ. Наблюдено в тот же день: строка `find` протухла после того,
// как правило переписали, движок отчитался «1 mutation survived», и следующим шагом был бы
// переписанный исправный харнесс.
{
  const { root, subject, harness } = fixture(
    (s) => `import assert from "node:assert/strict";\nimport { readFileSync } from "node:fs";\nassert.match(readFileSync(${JSON.stringify(s)}, "utf8"), /keep-me/, "MARKER пропал");\n`,
  );
    ...
    const { code, out } = silently(() =>
      runMutations({
        ...
        cases: [{
          name: "stale target",
          harness,
          expect: "MARKER пропал",
          disables: "фикстура",
          // Строки в файле нет — ровно случай протухшей цели.
          edits: [[subject, "no-such-string-anywhere", "x"]],
        }],
      }),
    );
    assert.equal(code, 1, `непригодная мутация обязана ОТКАЗЫВАТЬ прогон; получено ${code}\n${out}`);
    assert.match(out, /UNUSABLE/, "вердикт обязан называться непригодностью, а не выживанием");
    assert.match(out, /NOT FOUND/, "и называть ПРИЧИНУ — что именно не нашлось");
    assert.doesNotMatch(
      out,
      /mutation\(s\) survived/,
      "и НЕ заявлять выжившую мутацию: это отправило бы чинить исправный тест",
    );
    cases.push("патч не лёг → UNUSABLE с причиной, а не «мутация выжила»");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
}

// Отчитаться инструменту, иначе поверхность числится НЕПОКРЫТОЙ при работающем тесте —
// «0 checks» и «теста нет» в леджере выглядят одинаково.
