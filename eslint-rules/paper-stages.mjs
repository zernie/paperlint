...
export default {
  rules: {
     ...
     *
     * ⚠️ ЧЕГО ЭТО НЕ ЧИНИТ, названо прямо: свидетельство прогона остаётся ПРОЗОЙ — маркером в
     * ячейке-примечании. Иначе сформулированный прогон правило не увидит. Сузить поиск до строки
     * таблицы было бы можно, но замер 2026-09-17 показал, что разницы нет: во всех трёх статьях
     * маркер встречается ровно один раз и именно в строке сверки ссылок. Правило без измеренной
     * разницы не усложняем.
     *
      ...
      create(context) {
        ...
        return {
          yaml(node) {
            ...
            // Свидетельство ищем во ВСЁМ табеле, а не в разобранном узле: маркер живёт в прозе
            // ячейки-примечания, у которой своего узла нет.
            if (context.sourceCode.text.includes(marker)) return;
            context.report({
              node,
              messageId: "neverRan",
              data: {
