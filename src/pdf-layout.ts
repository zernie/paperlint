/**
 * WHERE EACH PIECE OF TEXT SITS ON EACH PAGE, and the same thing written as `pdftohtml -xml`
 * output — PURE. No disk, no pdf.js, no process.
 *
 ...
 * tool would count, and writes them in the XML dialect that banal (HotCRP's page-geometry script,
 * see `banal.ts`) reads. banal then measures that XML exactly as it measures the XML poppler's
