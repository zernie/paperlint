...
const PDF_BYTES = new TextEncoder().encode("%PDF-1.5 a stand-in for the built PDF");
...
function goodFacts(): Json {
  return {
    ...
    fonts: [
      { name: "LinLibertineT", type: "Type 1", embedded: true, program: "Type1" },
      { name: "LinBiolinumTB", type: "Type 1", embedded: true, program: "Type1" },
    ],
...
function lint(p: Paper, filename = `${PAPER}/paper.tex`, options: Json = {}): Finding[] {
  const files = memoryFiles({
    ...
    ...(p.venue === undefined
      ...
      : {
          [`${PAPER}/venue.json`]:
            typeof p.venue === "string" ? p.venue : JSON.stringify(p.venue),
        }),
    ...(p.facts === null
      ? {}
      : {
          [`${PAPER}/_build/paper.facts.json`]:
            typeof p.facts === "string"
              ? p.facts
              : JSON.stringify(p.facts ?? goodFacts()),
        }),
    ...(p.pdf === null ? {} : { [`${PAPER}/paper.pdf`]: p.pdf ?? PDF_BYTES }),
    ...(p.extra ?? {}),
  });
  const rules = venueRules({ files, venuesDir: VENUES });
  const out: Finding[] = [];
  for (const [name, rule] of Object.entries(rules) as [string, VenueRuleModule][]) {
    const visitor = rule.create({
      filename,
      sourceCode: { text: TEX },
      options: name === "geometry" && Object.keys(options).length ? [options] : [],
      report: (d) =>
        out.push({
          rule: `pdf/${name}`,
...
const ids = (fs: readonly Finding[]) => fs.map((f) => `${f.rule}:${f.messageId}`).sort();
...
describe("a paper that meets its venue", () => {
  ...
  it("takes the venue from venue.json, not from the facts: facts measured with no venue still judge", () => {
    expect(
      lint({ venue: DECL, facts: withFacts((f) => ((f.venue = null), (f.kind = null))) }),
    ).toEqual([]);
  });
});
...
describe("a paper that names no venue", () => {
  ...
  it("acts on paper.tex only — any other file gets nothing", () => {
    expect(lint({ venue: DECL, facts: null }, `${PAPER}/PIPELINE-STATUS.md`)).toEqual([]);
  });
});

describe("pdf/profile — the declaration must resolve, or nothing is judged", () => {
  it.each([
    ["an unknown venue (a typo would silently disable every check)", { venue: "agentic-dev", kind: "short" }, "venueUnknown", /agenticdev, aisec, realm/],
    ["the base TeX set is not a venue", { venue: "tex-base" }, "venueUnknown", /agenticdev, aisec, realm/],
    ["venue.json that is not JSON", "{ venue: agenticdev", "venueJsonBroken", /venue\.json/],
  ...
  it("a profile that does not parse is named with the file", () => {
    const fs = lint({
      ...
      extra: { [join(VENUES, "mine.jsonc")]: '{ "tex": { "packages": {} }, "columns": "two" }' },
    });
  ...
  it.each([
    ...
    ["a kind the venue does not have", { venue: "agenticdev", kind: "long" }, "kindUnknown"],
  ])("%s: the page limit is not checked — and the rest still is", (_, venue, messageId) => {
    ...
    const fonts = lint({ venue, facts: withFacts((f) => ((f.fonts as Json[])[0]!.embedded = false)) });
    expect(ids(fonts)).toEqual([`pdf/fonts:notEmbedded`, `pdf/profile:${messageId}`]);
  });
  ...
  it("a kind with no limits in the profile (realm's `long`) is not a finding", () => {
    const realm = withFacts((f) => {
      ...
      f.ref_pt = 10;
      f.body_pages = 30;
      f.fonts = [{ name: "NimbusRomNo9L-Regu", type: "Type 1", embedded: true, program: "Type1" }];
    });
    expect(lint({ venue: { venue: "realm", kind: "long" }, facts: realm })).toEqual([]);
  });
});
...
describe("pdf/measured — a paper that names a venue and was not measured says so (warn)", () => {
  ...
  it("facts without geometry (no banal): pdf/measured names it, the three geometry rules are silent, fonts still judge", () => {
    const g = withFacts((f) => {
      ...
      for (const k of ["page_w_in", "page_h_in", "columns", "body_pt", "ref_pt", "body_pages", "ref_pages", "appendix_pages", "pages_by_type"])
        f[k] = null;
      (f.fonts as Json[])[0]!.program = "Type3";
    });
    expect(ids(lint({ venue: DECL, facts: g }))).toEqual(["pdf/fonts:type3", "pdf/measured:noGeometry"]);
  });
});

describe("pdf/fresh — facts about another build are not judged", () => {
  it.each([
    ["a changed PDF", { pdf: new TextEncoder().encode("another build") }, "stale"],
    ...
    ["facts missing pdf_sha256", { facts: withFacts((f) => delete f.pdf_sha256) }, "factsBroken"],
    ["a font entry of the wrong shape", { facts: withFacts((f) => (f.fonts = [{ name: 3 }])) }, "factsBroken"],
  ] as [string, Paper, string][])("%s: one finding, from pdf/fresh only", (_, p, messageId) => {
    // Everything else planted wrong too — none of it may be reported over stale facts.
    const facts = p.facts ?? withFacts((f) => ((f.body_pages = 99), (f.columns = 1)));
    expect(ids(lint({ venue: DECL, ...p, facts }))).toEqual([`pdf/fresh:${messageId}`]);
  });
});

describe("pdf/fonts", () => {
  it.each([
    ["a Type 3 font", (f: Json) => ((f.fonts as Json[])[0]!.program = "Type3"), ["pdf/fonts:type3"]],
    ["a font not embedded", (f: Json) => ((f.fonts as Json[])[1]!.embedded = false), ["pdf/fonts:notEmbedded"]],
    [
    ...
    [
      ...
      (f: Json) => (f.fonts = [{ name: "CMR10", type: "Type 1", embedded: true, program: "Type1" }]),
  ...
  it("names the family and the fonts the PDF does have", () => {
    const [f] = lint({ venue: DECL, facts: withFacts((x) => ((x.fonts as Json[])[0]!.name = "CMR10")) });
...
describe("pdf/geometry", () => {
  it.each([
    ["A4 instead of letter", (f: Json) => ((f.page_w_in = 8.27), (f.page_h_in = 11.69)), ["pdf/geometry:dim", "pdf/geometry:dim"]],
    ["one column instead of two", (f: Json) => (f.columns = 1), ["pdf/geometry:columns"]],
    ["a page size banal could not read", (f: Json) => (f.page_w_in = null), ["pdf/geometry:dimMissing"]],
    ["0.03 in off is inside the default tolerance", (f: Json) => (f.page_w_in = 8.53), []],
  ] as [string, (f: Json) => void, string[]][])("%s", (_, patch, want) => {
    expect(ids(lint({ venue: DECL, facts: withFacts(patch) }))).toEqual(want);
  });
  ...
  it("dimTol widens the tolerance", () => {
    ...
    expect(lint({ venue: DECL, facts }, `${PAPER}/paper.tex`, { dimTol: 0.2 })).toEqual([]);
  });
});

describe("pdf/limits", () => {
  it.each([
    ["one body page over the short-paper limit", (f: Json) => (f.body_pages = 6), ["pdf/limits:pages"], /6 .*limit 5.*agenticdev\/short/],
    ["one reference page over", (f: Json) => (f.ref_pages = 3), ["pdf/limits:pages"], /3 .*limit 2/],
    ["exactly at the limit", (f: Json) => (f.body_pages = 5), [], null],
    ["a reference font below the range", (f: Json) => (f.ref_pt = 6), ["pdf/limits:refPt"], /6 pt/],
    ["6.8 pt is inside the range once the measuring drift (body_pt_tol) is allowed", (f: Json) => (f.ref_pt = 6.8), [], null],
    ["no reference font measured (no bibliography) is not a finding", (f: Json) => (f.ref_pt = null), [], null],
  ] as [string, (f: Json) => void, string[], RegExp | null][])("%s", (_, patch, want, text) => {
  ...
  it("a longer kind of the same venue passes the same page count", () => {
    ...
    expect(lint({ venue: { venue: "agenticdev", kind: "full" }, facts })).toEqual([]);
  });
});

describe("pdf/body-size", () => {
  it.each([
    ["10.2 pt against 9 ± 0.5", (f: Json) => (f.body_pt = 10.2), ["pdf/body-size:body"]],
    ["9.4 pt is inside the tolerance", (f: Json) => (f.body_pt = 9.4), []],
    ["no body size measured", (f: Json) => (f.body_pt = null), ["pdf/body-size:bodyMissing"]],
  ] as [string, (f: Json) => void, string[]][])("%s", (_, patch, want) => {
    expect(ids(lint({ venue: DECL, facts: withFacts(patch) }))).toEqual(want);
  });
});
