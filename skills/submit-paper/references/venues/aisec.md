...
**ACM CCS — a top-4 security conference** — so calibrate to a *harder* bar than a fresh workshop:
...
- **Types / tracks**: **full research papers**; **benchmark papers** (new evaluation frameworks /
  ...
  (systematization-of-knowledge) papers. Pick the track and pitch to *its* bar.
...
2. **Ethics / responsible disclosure — CRITICAL, a hard reject-vector.**
   ...
   - No supplementary slot → the anonymized artifact on **OSF** must itself be leak-clean (the released
     files, not just the PDF). A named-entity leak in the *artifact* is invisible to a paper-only read
