...
const MUTATIONS = [
  ...
  [
    "no-round-ledger",
    "reporting a paper with no round manifests",
  ...
  [
    "two-open-rounds",
    "reporting two manifests with no `closed:`",
    "if (open.length > 1) {",
    "if (false) {",
  ...
  [
    "no-open-round",
    "reporting edits made with every round closed",
    "if (dirty) {",
    "if (false) {",
  ...
  [
    "ratchet/push",
    "reporting growth past the declared budget",
    "if (grew > budget) {",
    "if (false) {",
  ...
  [
    "ratchet/budget",
    "honouring the declared budget (every round is charged against 0)",
  ...
  [
    "ratchet-cumulative",
    "the compounding check across all rounds",
    "if (cum > total) {",
    "if (false) {",
  ...
  [
    "hedge/push",
    "reporting hedge density growth",
    "if (dh > hb + 1e-9) {",
    "if (false) {",
  ...
  [
    "hedge/budget",
    "honouring the declared hedge budget",
  ...
  [
    "overbroad-scope",
    "reporting a `touches:` list that covers the paper",
  ...
  [
    "number/novelty",
    "the rule that a number already in the paper is free to restate",
  ...
  [
    "number/direction",
    "comparing against the BASE (the 'already present' set becomes the current one)",
    "const wasNums = numbers(base);",
    "const wasNums = numbers(now);",
  ...
  [
    "manifest/touches",
    "rejecting a manifest with no `touches:`",
  ...
  [
    "covers/nesting",
    "number-prefix nesting in `touches:` (4 no longer covers 4.1)",
    'num.startsWith(e + ".")',
    "false",
    "must cover §2.1",
  ],
  [
    "covers/text",
    "the substring rule for NON-numeric entries (`Appendix` covers nothing)",
    "return h.includes(e);",
    "return false;",
  ...
  [
    "split/appendix",
    "the body↔appendix boundary (everything counts as body)",
  ...
  [
    "census/appendix",
    "keeping appendix words out of the body count",
    "    bodyWords: bw,",
    '    bodyWords: bw + words(appendix.map((s) => s.text).join("\\n")),',
    "appendix growth under a zero body budget must be free",
  ],
  [
    "strip/comments",
    "stripping HTML comments before every measurement",
...
process.exit(
  runMutations({
    ...
    cases: MUTATIONS.map((m) => {
      ...
      const [replace, expect] =
        m.length === 4 ? [`false && ${find}`, m[3]] : [m[3], m[4]];
      return {
        name,
        disables,
