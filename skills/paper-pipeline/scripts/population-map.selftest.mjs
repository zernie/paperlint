...
const kinds = (md, reg) =>
  findings(md, reg).map((f) => `${f.kind}:${f.row?.print ?? f.print}`);
...
check(
  "a registry that matches the body is silent",
  kinds(
    doc(
...
check(
  "stale registry row fires",
  kinds(doc("The resolver ran over all 1,921 repositories."), REG).includes(
    "stale:1,836",
  ),
);
...
check(
  "a retired row does not fire",
  !kinds(
    doc("The resolver ran over all 1,921 repositories."),
    REG + "\ngold\t7,310\tretired\t\ta public gold set",
  ).includes("stale:7,310"),
);
...
check(
  "a row that is absent is stale, not badref",
  kinds(
...
check(
  '189 is not "printed" inside 1,189',
  kinds(
