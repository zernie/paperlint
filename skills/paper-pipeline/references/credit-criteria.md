# What makes a paper COUNT — the grading this pipeline applies (single source of truth)

Why an independent researcher writes papers at all: each one is **evidence in a review of a body of
work** — a credential file that someone else will read, score criterion by criterion, and either credit
or refuse. Every stage of the pipeline is graded against this, not against abstract "impact." Cite this
file from the other skills instead of re-deriving it.

⚠️ **Two things this file is NOT.** It is not legal advice — where a credential review is run by a
government or a professional body, the final call belongs to a qualified adviser in that system, and the
statutes, policy manuals and decided cases that ground the rules below belong in the author's own private
notes, not in a shared pipeline. And it is not a promise: what follows is how reviewers of a body of work
...
- **Original contribution of major significance** — driven by the paper's **citations / adoption**, not
  GitHub stars. Being funded, patented or published "will not necessarily establish… major significance";
...
3. **Venue choice** (`find-venue`): a peer-reviewed, **indexed** venue is the whole game; workshop newness
   is fine; **non-archival** workshops (no indexed proceedings) rank LOW. The prestige upgrade is
   `extend-paper` to a stronger venue later, not holding out now.
4. **Timeline** (`plan-paper-timeline`): **space submissions out**; do not cluster them into the month
   before a planned external filing. This is the FILING-SPACING rule, and this paragraph is its reason.
5. **Name consistency**: publish under ONE canonical (passport) name across every paper, and use an
   **ORCID** to tie all output to one identity regardless of how a venue spells the name. Where a form
   asks for a country, it usually means **residence**, not citizenship — the two differ for many authors,
   and the wrong one creates payment and sanctions friction that has nothing to do with the science.
6. **Judging**: after each submission, offer to review for the venue; keep both the invitation and the
   proof that you completed the review.
7. **The personal half stays private.** Which filing, on what schedule, under whose advice — that is the
