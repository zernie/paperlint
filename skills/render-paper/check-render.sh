...
#
# Лестница из двух кандидатов, потому что каталог ПЕРЕЕЗЖАЕТ: сегодня он внутри скилла в
# потребителе, после шага 5 — внутри пакета `research-paper-pipeline`. Порядок «пакет, потом
# локальный» намеренный: когда переезд состоится, пакет должен побеждать молча, а когда ещё
# нет — работать локальный, тоже молча. Громко становится только если НЕТ НИ ОДНОГО.
...
resolve_venues() {
  ...
  # 2) каталог скилла в потребителе (сегодняшнее состояние)
...
if [ "${1:-}" = "--print-venues" ]; then
  ...
  echo "✗ paper-guards.tex не найден НИ в пакете research-paper-pipeline, ни в .claude/skills/submit-paper/references/venues" >&2
